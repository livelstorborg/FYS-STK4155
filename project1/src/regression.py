...
class RegressionAnalysis:
    ...
    def __init__(self, data, degree, lam=None, eta=None, num_iters=None):
        ...
        # Initialize parameter storage (don't calculate here, do it in fit methods)
        ...
        # Initialize train metrics storage
    ...
    def fit_analytical(self):
        ...
        if self.lam is not None and self.lam > 0:
            self.theta_ridge_analytical = Ridge_parameters(self.X_train, self.y_train, self.lam)
    ...
    def fit_gradient_descent(self):
        ...
        if self.lam is not None and self.lam > 0:
            self.theta_ridge_gd, self.ridge_mse_history = gd_Ridge(
                self.X_train, self.y_train, self.eta, self.num_iters, self.lam, stopping_param=1e-6
            )
        elif self.lam is not None and self.lam == 0:
    ...
    def predict(self):
        ...
        if self.theta_ols_analytical is not None:
            self.y_pred_ols_analytical = self.X_test @ self.theta_ols_analytical + self.y_mean
            
        if self.theta_ridge_analytical is not None:
            self.y_pred_ridge_analytical = self.X_test @ self.theta_ridge_analytical + self.y_mean
    ...
    def calculate_metrics(self):
        ...
        if self.y_pred_ols_analytical is not None:
            # Test metrics
            self.mse_ols_analytical = mean_squared_error(y_test_true, self.y_pred_ols_analytical)
            ...
            self.train_mse_ols_analytical = mean_squared_error(y_train_true, y_train_pred_ols)
            self.train_r2_ols_analytical = r2_score(y_train_true, y_train_pred_ols)
            
        # Ridge Analytical
        if self.y_pred_ridge_analytical is not None:
            # Test metrics
            self.mse_ridge_analytical = mean_squared_error(y_test_true, self.y_pred_ridge_analytical)
            self.r2_ridge_analytical = r2_score(y_test_true, self.y_pred_ridge_analytical)
            # Train metrics
            y_train_pred_ridge = self.X_train @ self.theta_ridge_analytical + self.y_mean
            self.train_mse_ridge_analytical = mean_squared_error(y_train_true, y_train_pred_ridge)
        ...
        if self.y_pred_ols_gd is not None:
            ...
            self.train_mse_ols_gd = mean_squared_error(y_train_true, y_train_pred_ols_gd)
        ...
        if self.y_pred_ridge_gd is not None:
            ...
            self.train_mse_ridge_gd = mean_squared_error(y_train_true, y_train_pred_ridge_gd)
    ...
    def print_results(self):
        ...
        if self.mse_ols_analytical is not None:
            print(f"{'OLS (Analytical)':<20} {self.mse_ols_analytical:<12.6f} {self.r2_ols_analytical:<12.6f}")
        if self.mse_ols_gd is not None:
            print(f"{'OLS (Grad. Desc.)':<20} {self.mse_ols_gd:<12.6f} {self.r2_ols_gd:<12.6f}")
        if self.mse_ridge_analytical is not None:
            print(f"{'Ridge (Analytical)':<20} {self.mse_ridge_analytical:<12.6f} {self.r2_ridge_analytical:<12.6f}")
        if self.mse_ridge_gd is not None:
            print(f"{'Ridge (Grad. Desc.)':<20} {self.mse_ridge_gd:<12.6f} {self.r2_ridge_gd:<12.6f}")
