...
def mse_degree_ols(results_dict, sample_size):
    ...
    plt.plot(results['degrees'], results['train_mse'], 'o-', label='MSE (train)', linewidth=2, markersize=6)
    plt.plot(results['degrees'], results['test_mse'], 'o-', label='MSE (test)', linewidth=2, markersize=6)
...
def r2_degree_ols(results, sample_size):
    ...
    """
    plt.figure(figsize=(8,6))
    plt.plot(results['degrees'], results['train_r2'], 'o-', label='R² (train)', linewidth=2, markersize=6)
    plt.plot(results['degrees'], results['test_r2'], 'o-', label='R² (test)', linewidth=2, markersize=6)
    plt.xlabel('Polynomial Degree', fontsize=16)
...
def mse_degree_multiple(results_dict, sample_sizes):
    ...
    for N in sample_sizes:
        ...
        plt.plot(results['degrees'], results['test_mse'], 'o-', 
...
def mse_degree_lambdas(results_dict, lambda_values, sample_size):
    ...
    for lam in lambda_values:
        ...
        plt.plot(results['degrees'], results['test_mse'], 'o-', label=f'λ = {lam:.1e}', linewidth=2, markersize=4)
...
def r2_degree_lambdas(results_dict, lambda_values, sample_size):
    ...
    for lam in lambda_values:
        results = results_by_lambda[lam]
        plt.plot(results['degrees'], results['test_r2'], 'o-', label=f'λ = {lam:.1e}', linewidth=2, markersize=4)
...
def theta_evolution_lambdas(all_results, lambda_values, degrees, sample_size, include_ols=True):
    ...
    for lam in lambda_values:
        results = results_by_lambda[lam]
        theta1_values = [instance.theta_ridge_analytical[0] for instance in results['instances']]
        plt.plot(degrees, theta1_values, 'o-', label=f'Ridge λ = {lam:.1e}', linewidth=2, markersize=4)
    
    if include_ols:
        first_results = results_by_lambda[lambda_values[0]]
        ols_theta1_values = [instance.theta_ols_analytical[0] for instance in first_results['instances']]
        plt.plot(degrees, ols_theta1_values, '--', label='OLS (λ = 0)', linewidth=3, markersize=6)
...
def solution_comparison(x, solutions, sample_size, degree, lam):
    ...
    y_pred_ols_analytical, y_pred_ols_gd, y_pred_ridge_analytical, y_pred_ridge_gd, y_true = solutions
